...
create table if not exists firstday2004_2018 as 
	select t1.ticker,t1.sector,min(t1.data) as firstday
	from fullstocktable t1
	where year(t1.data)>=2004 and year(t1.data)<=2018
	group by t1.ticker,t1.sector,year(t1.data);

create table if not exists lastday2004_2018 as 
	select t1.ticker,t1.sector,max(t1.data) as lastday
	from fullstocktable t1
	where year(t1.data)>=2004 and year(t1.data)<=2018
	group by t1.ticker,t1.sector,year(t1.data);

create table if not exists volumebysector as 
...
select t1.sector,t1.annualvolume,t1.avgdailyquotation,t2.quotationincrease,t1.anno
	...
	join (select c.sector,((c.lastdayquotation-d.firstdayquotation)/d.firstdayquotation) as quotationincrease, c.anno
