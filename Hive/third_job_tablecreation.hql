...
create table if not exists firstday2016_2018 as 
	select t1.name,min(t1.data) as firstday,year(t1.data) as anno
	from fullstocktable t1
	where year(t1.data)>=2016 and year(t1.data)<=2018
	group by t1.name,year(t1.data);

create table if not exists lastday2016_2018 as 
	select t1.name,max(t1.data) as lastday,year(t1.data) as anno
	from fullstocktable t1
	where year(t1.data)>=2016 and year(t1.data)<=2018
	group by t1.name,year(t1.data);

create view if not exists firstdaycompanyquotation as
	select t1.name,t1.sector,avg(t1.close) as firstdayquotation,year(t1.data) as anno 
...
create view if not exists lastdaycompanyquotation as
	select t1.name,t1.sector,avg(t1.close) as lastdayquotation,year(t1.data) as anno
